...
import { hash, id, harnessPath } from "../storage.js";
...
const paths = [harnessPath("runtime/src"), harnessPath("runtime/tests"), harnessPath("schemas"), harnessPath("config"), harnessPath("roles"), harnessPath("cli"), harnessPath("mcp"), "AGENTS.md", "CLAUDE.md", harnessPath("README.md"), harnessPath("scripts/verify-phase-2-preservation.py"), harnessPath("artifacts/phase-2-completion-workspace-preservation.json")];
// Workspace governance and generated artifacts may be absent in a clean clone; snapshot records their absence.
const optionalPaths = ["AGENTS.md", "CLAUDE.md", harnessPath("artifacts/phase-2-completion-workspace-preservation.json")];
export function reviewSubject(bus: AgentBus, root: string) {
  ...
  const subject = { manifest: m, artifacts: new WorkflowService(bus).artifacts(root), ...(engineering.length?{engineering}:{}), harness_source_hashes: snapshot(paths, [], optionalPaths) };
...
export async function reviewWorkflow(bus: AgentBus, root: string, task: string, execute = false) {
  ...
  const capsule: TaskCapsule = {
    ...
    required_context: [harnessPath("runtime/src/workflow/service.ts"), harnessPath("runtime/src/workflow/artifacts.ts"), harnessPath("runtime/src/workflow/adapters.ts"), harnessPath("runtime/src/agent-bus/index.ts"), ...(hasEngineering(bus,root)?[harnessPath("runtime/src/workflow/engineering/service.ts"),harnessPath("runtime/src/workflow/engineering/artifacts.ts"),harnessPath("runtime/src/workflow/engineering/adapters.ts"),harnessPath("runtime/src/workflow/engineering/fixture.ts")]:[])],
    allowed_paths: [...new Set([...paths, ...m.request.allowed_paths, harnessPath("state/provider-observations"), harnessPath("state/events"), harnessPath("state/results"), harnessPath("state/workflows"), harnessPath("state/worktrees"), harnessPath("state/engineering-verifications"), harnessPath("state/ownership"), harnessPath("state/quarantine"), harnessPath("state/fixtures/registration.json"), harnessPath("artifacts/phase-2-workspace-preservation.json"), harnessPath("artifacts/phase-2-tests.tap"), harnessPath("artifacts/phase-2-completion-tests.tap")])],
